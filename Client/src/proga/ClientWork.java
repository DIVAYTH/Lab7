            ...
            if (finalUserCommand.length == 1) {
                switch (command) {
                    ...
                    case "":
                        break;
                    case "clear": {
                        Command request = new Command(finalUserCommand[0], login);
                        sendCommand(socket, request);
                        getAnswer(socket);
                    }
                    break;
                    ...
                    case "print_field_descending_form_of_education": {
                        Command request = new Command(finalUserCommand[0]);
                        sendCommand(socket, request);
                        getAnswer(socket);
                    }
                    break;
                    ...
                    case "add_if_min":
                    case "add": {
                        Command request = new Command(finalUserCommand[0], add(), login);
                        sendCommand(socket, request);
                        getAnswer(socket);
                    }
                    break;
                ...
                switch (finalUserCommand[0]) {
                    ...
                    case "update":
                        try {
                            Integer.parseInt(finalUserCommand[1]);
                            Command request = new Command(finalUserCommand[0], finalUserCommand[1], add(), login);
                            sendCommand(socket, request);
                            getAnswer(socket);
                        } catch (NumberFormatException e) {
                            System.out.println("Вы ввели строку или число выходит за пределы int. Введите снова");
